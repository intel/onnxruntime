...
bool EPCtxHandler::StartReadingContextBin(const std::filesystem::path& bin_file_path, openvino_ep::weight_info_map& shared_weight_info_) {
  ...
  //context_bin_stream_.seekg(header.compiled_model_map_pos);
  //read_bytes(context_bin_stream_, compiled_models_info_);
  read_bytes(context_bin_stream_, compiled_models_info_, header.compiled_model_map_pos);

  // Get weight map
  //context_bin_stream_.seekg(header.weight_map_pos);
  //read_bytes(context_bin_stream_, shared_weight_info_);
