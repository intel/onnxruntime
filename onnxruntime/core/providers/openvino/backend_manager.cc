...
//     return HandleConcreteModel(graph_inputs, is_npu_device); //Change the name of this function to something else
//   }
...
//   if (!session_context_.shape.empty()) {
//     try {
//       ValidateInputShapes(session_context_.shape, graph_inputs); //input1[1,3,5  [inpu1[1,3,4,5]]]
...
//       if (is_npu_device) {
//         session_context_.disable_dynamic_shapes = true; // Remove this thing
...
bool BackendManager::ModelHasSymbolicInputDims(const onnxruntime::GraphViewer& subgraph) const {
  ...
  // First pass: Count dynamic inputs and check if reshape covers all of them
  ...
  for (const auto* input : graph_inputs) {
    ...
    if (has_dynamic_dim) {
      ...
      if (!session_context_.shape.empty() && 
          session_context_.shape.find(input->Name()) == session_context_.shape.end()) {
        all_dynamic_inputs_covered = false;
        LOGS_DEFAULT(WARNING) << "[OpenVINO-EP] reshape_input is provided but doesn't cover dynamic input: "
                             << input->Name();
      }
  ...
  //  Handle concrete model case
  if (!has_symbolic_dims) {
    if (!session_context_.shape.empty()) {
      try {
        ValidateInputShapes(session_context_.shape, graph_inputs); // Validate the layout of the input shapes
        session_context_.disable_dynamic_shapes = false;
      } catch (const std::exception& e) {
        LOGS_DEFAULT(ERROR) << "[OpenVINO-EP] Shape validation failed: " << e.what();
        session_context_.shape.clear();   // Clear the shape map as it's invalid
  ...
  // Case: No reshape input provided for dynamic model
  if (session_context_.shape.empty()) {
    if (is_npu_device) {
      session_context_.disable_dynamic_shapes = true;
    }
  ...
  // Handle incomplete reshape coverage
  if (!all_dynamic_inputs_covered && !session_context_.shape.empty()) {
    session_context_.shape.clear();
    LOGS_DEFAULT(WARNING) << "[OpenVINO-EP] reshape_input does not cover all dynamic dimensions, "
                          << "ignoring all provided shapes";
  ...
  // Handle complete reshape coverage - validate shape compatibility
  try {
    ValidateInputShapes(session_context_.shape, graph_inputs);
