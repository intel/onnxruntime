...
std::unique_ptr<OrtValue> OrtValueFromShapeAndTypeWithMemoryInfo(const std::vector<int64_t>& shape,
  ...
  if (memory_info.device.Type() == OrtDevice::CPU) {
    allocator = GetAllocator();
  } else {
#if !defined(ORT_MINIMAL_BUILD)
    // Look up using the full OrtMemoryInfo from the EP device. This matches on mem_type + device,
    // which is required because HOST_ACCESSIBLE allocators register with OrtMemTypeCPU.
    auto& env = GetOrtEnv()->GetEnvironment();
    allocator = env.GetRegisteredSharedAllocator(memory_info);

    // Fallback: try the device-only lookup which has DEFAULT -> HOST_ACCESSIBLE fallback logic.
    if (!allocator) {
      allocator = GetSharedAllocator(memory_info.device);
    }
#endif
  }
